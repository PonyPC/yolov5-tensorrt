...
class Processor():
	def __init__(self, model):
		...
		self.nl = len(anchors)
		na = len(anchors[0]) // 2
		self.no = na * (self.nc + 5) # outputs per anchor
		self.strides = np.array([8., 16., 32.])
		self.output_shapes = [
			(self.batch_size, 3, 80, 80, self.no),
			(self.batch_size, 3, 40, 40, self.no),
			(self.batch_size, 3, 20, 20, self.no)
		]
		a = anchors.copy().astype(np.float32)
		self.anchor_grid = a.reshape(self.nl, 1, -1, 1, 1, 2)
	...
	def extract_boxes(self, output, conf_thres=0.5):
		...
		for out, grid, stride, anchor in zip(scaled, grids, self.strides, self.anchor_grid):
			...
			out = out.reshape((self.batch_size, 3 * width * height, self.no))
			z.append(out)
	...
	def post_process(self, outputs, conf_thres=0.5):
		...
		for i in range(pred.shape[0]):
			...
			boxes, confs, classes = self.nms(pi)
			box.append(boxes)
			conf.append(confs)
			clas.append(classes)
		return box, conf, clas

	def xywh2xyxy(self, x): #ok
	...
	def non_max_suppression(self, boxes, confs, classes, iou_thres=0.6):
		...
		while order.size > 0:
			...
			iother = order[1:]
			keep.append(i)
			xx1 = np.maximum(x1[i], x1[iother])
			yy1 = np.maximum(y1[i], y1[iother])
			xx2 = np.minimum(x2[i], x2[iother])
			yy2 = np.minimum(y2[i], y2[iother])
			w = np.maximum(0.0, xx2 - xx1 + 1)
			h = np.maximum(0.0, yy2 - yy1 + 1)
			inter = w * h
			ovr = inter / (areas[i] + areas[iother] - inter)
